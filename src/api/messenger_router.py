...
async def create_message(message: CreateMessage = Body(...), service: MessageService = Depends(get_message_service)):
    return await service.create_message(body=message)


@messenger_router.get("/messages/{topic_id}/{message_id}", response_model=ShowMessage, tags=["Мessages"])
async def get_message(topic_id: UUID, message_id: UUID, service: MessageService = Depends(get_message_service)):
...
async def get_last_messages_of_topic(topic_id: UUID, limit: int = 30, service: TopicService = Depends(get_topic_service)):
    return await service.get_last_messages_of_topic(body=TopicLimit(topic_id=topic_id, limit=limit))
